...
class UniversalXYConverterDialog(QDialog):
    ...
    def setup_crs_definitions(self):
        ...
        # Vérifier que le CRS est valide
        if not self.bftm_crs.isValid():
            self.iface.messageBar().pushMessage(
                "Erreur",
                "Échec de création du CRS BFTM",
                level=2
            )
    ...
    def setup_simple_tab(self):
        """Configure l'onglet de conversion simple."""
        layout = QVBoxLayout()
        ...
        self.source_crs_combo.currentIndexChanged.connect(
            self.on_source_crs_changed)
        ...
        self.custom_source_edit.setPlaceholderText(
            "ex: +proj=longlat +ellps=clrk80 +towgs84=-118,-14,218")
        ...
        self.target_crs_combo.currentIndexChanged.connect(
            self.on_target_crs_changed)
        ...
        self.custom_target_edit.setPlaceholderText(
            "ex: +proj=utm +zone=30 +ellps=GRS80 +units=m")
    ...
    def get_source_crs(self):
        ...
        return self.get_crs_from_combo(
            self.source_crs_combo,
            self.custom_source_edit,
            is_source=True)

    def get_target_crs(self):
        """Récupère le CRS cible."""
        return self.get_crs_from_combo(
            self.target_crs_combo,
            self.custom_target_edit,
            is_source=False)
    ...
    def convert_coordinates(self):
        ...
        try:
            if not self.source_x.text() or not self.source_y.text():
                self.iface.messageBar().pushMessage(
                    "Erreur", "Veuillez entrer des coordonnées", level=2)
                return
            ...
            self.iface.messageBar().pushMessage(
                "Succès", f"Conversion de {source_name} vers {target_name} réussie", level=1)

        except ValueError as e:
            self.conversion_status.setText(
                f"✗ Coordonnées invalides: {str(e)}")
            self.conversion_status.setStyleSheet("color: red;")
    ...
    def setup_batch_tab(self):
        ...
        self.source_file_path.setPlaceholderText(
        ...
        crs_layout.addWidget(
            QLabel("Système source (celui des coordonnées dans le fichier):"),
            0,
            0,
            1,
            2)
        ...
        self.set_combo_by_name(
            self.batch_source_crs_combo,
            "🌍 WGS 84 (degrés)")
        crs_layout.addWidget(self.batch_source_crs_combo, 1, 0, 1, 2)

        self.batch_custom_source_label = QLabel(
            "Chaîne PROJ personnalisée (source):")
        self.batch_custom_source_edit = QLineEdit()
        self.batch_custom_source_edit.setPlaceholderText(
            "ex: +proj=longlat +ellps=clrk80 +towgs84=-118,-14,218")
        ...
        crs_layout.addWidget(
            QLabel("Système cible (celui souhaité en sortie):"), 3, 0, 1, 2)
        self.batch_target_crs_combo = QComboBox()
        self.populate_crs_combo(self.batch_target_crs_combo)
        self.set_combo_by_name(
            self.batch_target_crs_combo,
            "🇧🇫 BFTM (Burkina Faso) - OFFICIEL")
        crs_layout.addWidget(self.batch_target_crs_combo, 4, 0, 1, 2)

        self.batch_custom_target_label = QLabel(
            "Chaîne PROJ personnalisée (cible):")
        self.batch_custom_target_edit = QLineEdit()
        self.batch_custom_target_edit.setPlaceholderText(
            "ex: +proj=utm +zone=30 +ellps=GRS80 +units=m")
        ...
        self.batch_source_crs_combo.currentIndexChanged.connect(
            self.on_batch_source_crs_changed)
        ...
        self.batch_add_original = QCheckBox(
            "Conserver les colonnes originales dans le fichier de sortie")
        self.batch_add_original.setChecked(True)
        options_layout.addWidget(self.batch_add_original)

        self.batch_add_validation = QCheckBox(
        ...
        self.batch_skip_invalid = QCheckBox(
    ...
    def get_batch_source_crs(self):
        ...
        if selected == "--- Personnalisé (PROJ) ---":
            ...
            if custom_crs.isValid():
                ...
                self.iface.messageBar().pushMessage(
                    "Erreur", "CRS source personnalisé invalide", level=2)
                return None
        else:
    ...
    def get_batch_target_crs(self):
        ...
        if selected == "--- Personnalisé (PROJ) ---":
            ...
            if custom_crs.isValid():
                ...
                self.iface.messageBar().pushMessage(
                    "Erreur", "CRS cible personnalisé invalide", level=2)
                return None
        else:
            return self.available_crs.get(selected)
    ...
    def process_batch(self):
        ...
        if not x_col or not y_col:
            QMessageBox.warning(
                self,
                "Attention",
                "Veuillez sélectionner les colonnes X et Y")
            return
        ...
        self.log_text.append(
            f"🔄 Conversion: {
                self.batch_source_crs_combo.currentText()} → {
                self.batch_target_crs_combo.currentText()}")
        self.log_text.append("")
    ...
    def batch_finished(self, output_path):
        ...
        if output_path:
            ...
            QMessageBox.critical(
                self,
                "Erreur",
                "Le traitement a échoué. Vérifiez le journal pour plus de détails.")
    ...
    def setup_interactive_tab(self):
        ...
        info_label = QLabel(
            "Cliquez sur un point de la carte pour obtenir ses coordonnées dans le système choisi")
        ...
        self.deactivate_picker_btn.clicked.connect(
            self.deactivate_point_picker)
        ...
        self.copy_interactive_btn.clicked.connect(
            self.copy_interactive_results)
    ...
    def activate_point_picker(self):
        ...
        self.iface.messageBar().pushMessage(
            "Info", "Cliquez sur la carte pour sélectionner un point", level=1)
    ...
    def on_map_click(self, point, button):
        ...
        try:
            ...
            if target_crs_text == "--- Personnalisé (PROJ) ---":
                ...
                target_crs = self.available_crs.get(
                    target_crs_text, self.bftm_crs)

            self.interactive_source.setText(
